# loss的创建，对loss 的创建和计算。2021/9/18
import tensorflow as tf
from tensorflow.keras import backend as K
from nets.yolo4_tiny import yolo_head
import math
#---------------------------------------------------#
#   平滑标签
#---------------------------------------------------#
...
# #---------------------------------------------------#
# #   将预测值的每个特征层调成真实值
# # 重点 难点 解码的过程  从85个维度特征层取出各特征层的真实值
# #---------------------------------------------------#
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, label_smoothing=0.1,
              print_loss=False, normalize=True):
    ...
    #---------------------------------------------------------------------------------------------------#
    y_true = args[num_layers:]#前面两层拿来做真实值，
    yolo_outputs = args [:num_layers]# 后面两层拿来做输出预测值
    #-----------------------------------------------------------#
